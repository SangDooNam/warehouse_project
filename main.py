...
class Controller:
    ...
    def answers(self):
        while True:
            ...
            if answer == 4:
                ...
                house_number = get_integer(f'Please select the house number of the warehouse(1 - {len(warehouse_list)}): ')
    ...
    def __search_item(self, search_term:str):
        ...
        if results:
            results = {key :[i for i in results if i.warehouse == key] for key in warehouse_ids}
            for key, value in results.items():
                for product in value:
                    date_of_stock = dt.strptime(product.date_of_stock, '%Y-%m-%d %H:%M:%S')
                    date_of_stock = dt.now() - date_of_stock
                    print(f'- {search_term.capitalize()} (in stock for {date_of_stock.days} days)')

                if len(value) == 1:        
                    print(f'In warehouse {key}, there is a total of {len(value)} {search_term}.')
                elif len(value) > 1:
                    print(f'In warehouse {key}, there is a total of {len(value)} {search_term}s.')
    ...
    def __order(self, search_term, total_amount):
        ...
        if order_prompt == 'n':
            ...
            if self.__authentication:
                while True:
                    order_amount = get_integer(f"How many would you like? (total amount:{total_amount}): ")
                    if order_amount > 0 and order_amount <= total_amount:
                        if order_amount == 1:
                            print(f"{order_amount} {search_term.capitalize()} has been ordered")
                            print(f'Thank you {self.name} for the order')
                            return f"Ordered a {search_term.capitalize()}"
                        else:
                            print(f"{order_amount} {search_term.capitalize()}s have been ordered")
                            print(f'Thank you {self.name} for the order')
                            return f"Ordered {order_amount} {search_term.capitalize()}s"
                    else:
                        print(f"This amount '{order_amount}' is invalid for the order")
                        ask_one_more_time = input("Press any key to continue with your order, or press 'q' to quit: ")
    ...
    def __select(self):
        ...
        dictionary = {idxs: (item_category, amount) for idxs, (item_category, amount) in enumerate(sorted(dictionary.items()), 1)}
        return dictionary
